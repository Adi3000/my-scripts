return {
	...
	execute = function(domoticz, device)
        	...
        	local upstairsCheckerDevice = domoticz.devices("Mode dodo")
  	        if ( upstairsCheckerDevice.state == "On" and device.name == 'PiZiGate - Couloir (Temp)')
  	        then
  	            checkHeating = true
  	        elseif ( downstairsCheckerDevice.state == "On" and device.name == 'PiZiGate - Salon (Temp)' )
  	        then
  	            checkHeating = true
  	        end
  	        if(checkHeating == true and device.temperature ~= 0 and device.temperature ~= 80)
  	        then
        		...
        		if ( device.temperature < minTemp and presenceDevice.state == "On" )
                then
                ...
                elseif ( device.temperature >= maxTemp )
                then
