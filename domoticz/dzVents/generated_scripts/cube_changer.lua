...
function addColor(domoticz, deviceName, addHue)
    ...
    if( newHue + oldHue > newHue - oldHue )
    then
...
return {
	...
	execute = function(domoticz, device)
        ...
        if(targetedDevice.getColor() ~= nil)
        then
            if ( device.sValue == 'Flip_180' )
            then
                if(targetedDevice.getColor().br ~= nil and targetedDevice.getColor().br > 0)
                then
