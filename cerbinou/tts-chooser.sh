...
curl -s -X POST 'https://rhasspy.adi3000.com/api/text-to-speech' --data-raw "$input_text" --connect-timeout 10 --output $cloud_filename 2> /dev/null &
cloud_pid=$!
...
curl -s -X POST 'http://192.168.0.21:8381/convert/tts' -H "Content-Type: application/json" --data-raw "{\"text\":\"$input_text\",\"speed\":\"1.2\"}" --connect-timeout 10 --output $melotts_filename 2> /dev/null &
melo_pid=$!
